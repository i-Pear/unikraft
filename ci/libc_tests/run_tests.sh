sudo apt-get update -y && sudo apt-get install -y qemu-system-x86-64
...
qemu-system-x86_64 -kernel "build/app-helloworld_qemu-x86_64" -nographic -cpu Skylake-Server > ../test_results.txt
